...
def process_image(image):
    ...
    cv2.imwrite(f'image_from_proc', image)
    ...
    for detected in result_dict:
        ...
        print("-----------------------------------")
        point = Pointxyz(real_coord_x, real_coord_y, center_depth[0][2])#nel robot vero devo prendere la coordinata della depth giusta: [0][depth_axis]
...
def calculate_depth(x, y):
    ...
    print("(calulate_depth) point list from depth: ", points_list)
...
def from_raw_to_jpg_compliant():
    ...
    image_arr = []
    while i < tot_data / (960 * 4):
        riga = []
        j = 0
        while j < tot_data / (540 * 4):
            riga.append(struct.unpack_from('3B', raw_image.data, int(j * 4 + i * tot_data / 540)))
            j += 1
